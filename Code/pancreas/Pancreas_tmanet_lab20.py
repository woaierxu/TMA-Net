...
def pretrain(net_bcp, net_nobcp, optimizer1, optimizer2, lab_loader_a, lab_loader_b, test_loader):
    ...
    for epoch in tqdm_load(range(1, pretraining_epochs + 1), ncols=70):
        if epoch % 5 == 0:
            net_bcp.eval()
            net_nobcp.eval()
            avg_metric1, _ = test_calculate_metric(net_bcp, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed)
            avg_metric2, _ = test_calculate_metric(
                net_nobcp, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed
            )
    ...
    for i_epoch, epoch in enumerate(tqdm_load(range(1, self_training_epochs + 1))):
        ...
        if (epoch % 20 == 0) | ((epoch >= 160) & (epoch % 5 == 0)):
            ...
            avg_metric3, _ = test_calculate_metric_mean(
                net_bcp, net_nobcp, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed
            )
...
def test_model(net1, net2, test_loader):
    ...
    avg_metric, _ = test_calculate_metric(net1, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed)
    avg_metric2, _ = test_calculate_metric(net2, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed)
    avg_metric3, _ = test_calculate_metric_mean(net1, net2, test_loader.dataset, s_xy=16, s_z=4, text_embed=text_embed)
    print(avg_metric)
    print(avg_metric2)
    print(avg_metric3)
