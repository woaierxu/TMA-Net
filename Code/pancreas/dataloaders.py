...
def get_dataset_path(dataset='pancreas', labelp='10percent'):
    tag = '_per10' if labelp == '10percent' else '_per20'
...
def get_ema_model_and_dataloader(data_root, split_name, batch_size, lr, labelp=10):
    ...
    net1 = create_Vnet()
    net2 = create_ResNet()
    ema_net1 = create_Vnet(ema=True).cuda()
    ...
    lab_loader_a = DataLoader(trainset_lab_a, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)

    trainset_lab_b = Pancreas(data_root, split_name, split='train_lab', labelp=labelp, reverse=True)
    lab_loader_b = DataLoader(trainset_lab_b, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)
    
    trainset_unlab_a = Pancreas(data_root, split_name, split='train_unlab', labelp=labelp)
    unlab_loader_a = DataLoader(trainset_unlab_a, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)

    trainset_unlab_b = Pancreas(data_root, split_name, split='train_unlab', labelp=labelp, reverse=True)
    unlab_loader_b = DataLoader(trainset_unlab_b, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)
