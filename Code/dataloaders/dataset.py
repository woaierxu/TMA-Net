...
class ACDCDataSet(Dataset):
    def __init__(self, base_dir=None, split='train_lab', transform=None, reverse=False, logging=None):
        ...
        if 'train_lab' in self.split :
            with open(f'./Datasets/acdc/data_split/{split}.list', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif 'train_unlab' in self.split:
            with open(f'./Datasets/acdc/data_split/{split}.list', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]

        elif self.split == 'val':
            with open('./Datasets/acdc/data_split/val.list', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]
        elif self.split == 'test':
            with open('./Datasets/acdc/data_split/test.list', 'r') as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]
        # if num is not None and self.split == "train":
        #    self.sample_list = self.sample_list[:num]
    ...
    def __getitem__(self, idx):
        ...
        if ("train_lab" in self.split) | ("train_unlab" in self.split):
            sample = self.transform(sample)
        # sample["idx"] = idx
        # sample['case'] = case
